...
static int chiron_utimens(const char *path_orig, const struct timespec tv[2],
			  struct fuse_file_info *fi)
{
	(void)fi;
	char   *fname;
	...
	for(i = 0; i < config.max_replica; i++) {
		...
		res = utimensat(AT_FDCWD, fname, tv, AT_SYMLINK_NOFOLLOW);
		if (res < 0) {
			err_list[i] = -errno;
			fail_cnt++;
			free(fname);
	...
	disable_faulty_replicas("utimens", succ_cnt, fail_cnt, err_list);
	...
	gettmday(&t2, NULL);
	timeval_subtract(&t3, &t2, &t1);
	dbg("utimens time %ld secs, %ld usecs\n", t3.tv_sec, t3.tv_usec);
...
static int chiron_rename(const char *from, const char *to, unsigned int flags)
{
	if (flags != 0) {
		return -EINVAL;
	}
 ...
 */
void *chiron_init(struct fuse_conn_info *conn, struct fuse_config *cfg)
{
	pthread_t thand;
	(void)conn;
	(void)cfg;
...
{
	printf("chiron3FS version: %s\n", PACKAGE_VERSION);
}


void help(void)
{
	fprintf(stderr, "usage: chiron3fs [OPTIONS] path=path[=path[=path...]] mountpoint\n");
